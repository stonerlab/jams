...
#ifndef JBLIB_CONTAINERS_ARRAY4D_H
#define JBLIB_CONTAINERS_ARRAY4D_H
...
//-----------------------------------------------------------------------------
  ...
  : size0_(size0), size1_(size1), size2_(size2), size3_(size3),
    data_(size0_*size1_*size2_*size3_ ? reinterpret_cast<pointer>
      (allocate_aligned64(size0_*size1_*size2_*size3_*sizeof(Tp_))): NULL)
  {}
...
//-----------------------------------------------------------------------------
  ...
  template <typename Tp_, typename Idx_>
  Array<Tp_, 4, Idx_>& Array<Tp_, 4, Idx_>::
  operator=(Array<Tp_, 4, Idx_> rhs) {
    using std::swap;
    swap(*this, rhs);
    return *this;
  }
  ...
  template <typename Tp_, typename Idx_>
  inline typename Array<Tp_, 4, Idx_>::reference
  Array<Tp_, 4, Idx_>::
  ...
  template <typename Tp_, typename Idx_>
  inline typename Array<Tp_, 4, Idx_>::const_reference
  Array<Tp_, 4, Idx_>::
  operator() (const size_type i, const size_type j, const size_type k,
...
//-----------------------------------------------------------------------------
  ...
  template <typename Tp_, typename Idx_>
  inline typename Array<Tp_, 4, Idx_>::pointer
  Array<Tp_, 4, Idx_>::
  data() {
    assert(is_allocated());
    return data_;
  ...
  template <typename Tp_, typename Idx_>
  inline typename Array<Tp_, 4, Idx_>::const_pointer
  Array<Tp_, 4, Idx_>::
  data() const {
    assert(is_allocated());
    return data_;
  ...
  template <typename Tp_, typename Idx_>
  inline const typename Array<Tp_, 4, Idx_>::size_type&
  Array<Tp_, 4, Idx_>::
  ...
  resize(const size_type new_size0, const size_type new_size1,
    ...
    if ((new_size0 == size0_) && (new_size1 == size1_) && (new_size2 == size2_) && (new_size3 == size3_)) {
      return;
    }

    Array< Tp_, 4, Idx_> newArray(new_size0, new_size1, new_size2, new_size3);
      ...
      for (size_type j = 0, jend = std::min(new_size1, size1_); j != jend; ++j) {
        for (size_type k = 0, kend = std::min(new_size2, size2_); k != kend; ++k) {
          for (size_type l = 0, lend = std::min(new_size3, size3_); l != lend; ++l) {
            newArray(i, j, k, l) = data_[((i*size1_+j)*size2_+k)*size3_+l];
          }
        }
      }
    }
...
//-----------------------------------------------------------------------------
  ...
  template <typename Tp_, typename Idx_>
  void swap(Array<Tp_, 4, Idx_>& first, Array<Tp_, 4, Idx_>& second) {
    using std::swap;
...
#endif  // JBLIB_CONTAINERS_ARRAY4D_H
