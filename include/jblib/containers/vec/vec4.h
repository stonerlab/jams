...
#ifndef JBLIB_CONTAINERS_VEC4_H
#define JBLIB_CONTAINERS_VEC4_H
...
namespace jblib {
  ...
  template< typename valueType_ >
    inline void Vec4<valueType_>::set(const valueType_ ix, const valueType_ iy, const valueType_ iz, const valueType_ iw){
      x = ix; y = iy; z = iz; w = iw;
    }

  template< typename valueType_ >
    inline valueType_ Vec4<valueType_>::operator[]( const int32 i ) const{
      return (&x)[i];
    }

  template< typename valueType_ >
    inline valueType_ & Vec4<valueType_>::operator[]( const int32 i ){
      return (&x)[i];
    }

  template< typename valueType_ >
    inline Vec4<valueType_> Vec4<valueType_>::operator-() const {
      return Vec4<valueType_>( -x, -y, -z, -w);
    }

  //template< typename valueType_ >
  //inline Vec4<valueType_> & Vec4<valueType_>::operator=(const Vec4<valueType_> &a) {
  //x = a.x; y = a.y, z = a.z;
  //return *this;
  //}

  template< typename valueType_ >
    inline Vec4<valueType_> Vec4<valueType_>::operator*(const valueType_ a) const{
  ...
  template< typename valueType_ >
    inline Vec4<valueType_> Vec4<valueType_>::operator+(const Vec4<valueType_> &a) const{
  ...
  template< typename valueType_ >
    inline Vec4<valueType_> &Vec4<valueType_>::operator/=(const double a) {
      x /= a; y /= a; z /= a; w /= a;
      return *this;
    }
  ...
  //template< typename valueType_ >
  //inline Vec4<valueType_>& Vec4<valueType_>::operator=(Vec4<valueType_> rhs){
  //swap(*this, rhs);
  //return *this;
  //}
  ...
  // Friends
  template< typename valueType_ >
    inline void swap( const Vec4<valueType_> &a, const Vec4<valueType_> &b){
      std::swap(a.x,b.x); std::swap(a.y,b.y); std::swap(a.z,b.z); std::swap(a.w,b.w);
    }

  template< typename valueType_ >
    inline valueType_ dot( const Vec4<valueType_> &a, const Vec4<valueType_> &b){
      return (a.x*b.x + a.y*b.y + a.z*b.z + a.w*b.w);
    }

  //template< typename valueType_ >
    //inline Vec4<valueType_>& cross( const Vec4<valueType_> &a, const Vec4<valueType_> &b){
      //return Vec4<valueType_>( (a.y*b.z-a.z*b.y), (a.z*b.x-a.x*b.z), (a.x*b.y - a.y*b.x) );
    //}

  template< typename valueType_ >
    inline double abs( const Vec4<valueType_> &a){
      return sqrt( dot(a,a) );
    }
  ...
  //template< typename valueType_ >
    //inline double angle( const Vec4<valueType_> &a, const Vec4<valueType_> &b){
  ...
  ///
  inline int vecCountZeros(const Vec4<double> &a){
    return ( floats_are_equal(a.x,0.0) + floats_are_equal(a.y,0.0) + floats_are_equal(a.z,0.0) + floats_are_equal(a.w,0.0) );
  }
