...
[[gnu::hot]]
jams::Vec<double, 3> minimum_image_orthogonal_basis(const Cell& cell, const jams::Vec<double, 3>& r_cart_i, const jams::Vec<double, 3>& r_cart_j) {
  //   W. Smith, CCP5 Information Quarterly for Computer Simulation of Condensed Phases (1989).
  jams::Vec<double, 3> dr = cell.inverse_matrix() * (r_cart_j - r_cart_i);
...
[[gnu::hot]]
jams::Vec<double, 3> minimum_image_bruteforce(const Cell& cell, const jams::Vec<double, 3>& r_i_cart, const jams::Vec<double, 3>& r_j_cart) {
  ...
  jams::Vec<double, 3> r_ij_min = r_ij;
...
jams::Vec<double, 3> minimum_image(const Cell& cell, const jams::Vec<double, 3>& r_cart_i, const jams::Vec<double, 3>& r_cart_j) {
...
Cell scale(const Cell &cell, const jams::Vec<int, 3>& size) {
  jams::Vec<double, 3> new_a = cell.a1() * double(size[0]);
  jams::Vec<double, 3> new_b = cell.a2() * double(size[1]);
  jams::Vec<double, 3> new_c = cell.a3() * double(size[2]);
  return Cell(new_a, new_b, new_c, cell.periodic());
}

Cell rotate(const Cell &cell, const jams::Mat<double, 3, 3>& rotation_matrix) {
  jams::Vec<double, 3> new_a = rotation_matrix * cell.a1();
  jams::Vec<double, 3> new_b = rotation_matrix * cell.a2();
  jams::Vec<double, 3> new_c = rotation_matrix * cell.a3();
