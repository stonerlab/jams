...
void jams::InitBlochDomainWall::execute(const libconfig::Setting &settings) {
  ...
  jams::Vec<double, 3> normal = jams::normalize(jams::config_optional<jams::Vec<double, 3>>(settings, "normal", {1, 0, 0}));
  jams::Vec<double, 3> domain = jams::normalize(jams::config_optional<jams::Vec<double, 3>>(settings, "domain", {0, 0, 1}));
  ...
  for (auto i = 0; i < globals::num_spins; ++i) {
    ...
    jams::Vec<double, 3> m = {0, 1.0 / std::cosh(kPi * x / width), std::tanh(kPi * x / width)};
