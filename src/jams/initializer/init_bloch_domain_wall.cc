...
void jams::InitBlochDomainWall::execute(const libconfig::Setting &settings) {
  ...
  Vec<double, 3> normal = jams::normalize(jams::config_optional<Vec<double, 3>>(settings, "normal", {1, 0, 0}));
  Vec<double, 3> domain = jams::normalize(jams::config_optional<Vec<double, 3>>(settings, "domain", {0, 0, 1}));
  ...
  for (auto i = 0; i < globals::num_spins; ++i) {
    ...
    Vec<double, 3> m = {0, 1.0 / std::cosh(kPi * x / width), std::tanh(kPi * x / width)};
