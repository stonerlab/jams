...
__global__ void cuda_cubic_energy_kernel(const int num_spins, const unsigned * order,
                                         const jams::Real * magnitude, const jams::Real * axis1, const jams::Real * axis2, const jams::Real * axis3, const jams::Real * dev_s, jams::Real * dev_e) {
  const unsigned int idx = blockIdx.x*blockDim.x+threadIdx.x;
  const unsigned int base = 3u * idx;
  if (idx >= num_spins) return;

  const jams::Real3 s{dev_s[base + 0], dev_s[base + 1], dev_s[base + 2]};
...
__global__ void cuda_cubic_field_kernel(const int num_spins, const unsigned * order,
                                        const jams::Real * magnitude, const jams::Real * axis1, const jams::Real * axis2, const jams::Real * axis3, const jams::Real * dev_s, jams::Real * dev_h) {
  const unsigned int idx = blockIdx.x*blockDim.x+threadIdx.x;
  const unsigned int base = 3u * idx;
  if (idx >= num_spins) return;
...
jams::Real CudaCubicAnisotropyHamiltonian::calculate_total_energy(jams::Real time, const jams::MultiArray<jams::Real, 2>& spins) {
    calculate_energies(time, spins);
    return scalar_field_reduce_cuda(energy_, cuda_stream_.get());
}

void CudaCubicAnisotropyHamiltonian::calculate_energies(jams::Real time, const jams::MultiArray<jams::Real, 2>& spins) {
    cuda_cubic_energy_kernel<<<(globals::num_spins + dev_blocksize_ - 1) / dev_blocksize_, dev_blocksize_, 0, cuda_stream_.get()>>>
            (globals::num_spins, order_.device_data(), magnitude_.device_data(), u_axes_.device_data(),
             v_axes_.device_data(), w_axes_.device_data(), spins.device_data(), energy_.mutable_device_data());
    DEBUG_CHECK_CUDA_ASYNC_STATUS;
}


void CudaCubicAnisotropyHamiltonian::calculate_fields(jams::Real time, const jams::MultiArray<jams::Real, 2>& spins) {
        cuda_cubic_field_kernel<<<(globals::num_spins+dev_blocksize_-1)/dev_blocksize_, dev_blocksize_, 0, cuda_stream_.get()>>>
                (globals::num_spins, order_.device_data(), magnitude_.device_data(), u_axes_.device_data(),
                 v_axes_.device_data(), w_axes_.device_data(), spins.device_data(), field_.mutable_device_data());
        DEBUG_CHECK_CUDA_ASYNC_STATUS;
}
