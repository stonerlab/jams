...
struct CubicAnisotropySetting {
    unsigned order = 1;
    double energy = 0.0;
    Vec<double, 3> u = {1.0, 0.0, 0.0};
    Vec<double, 3> v = {0.0, 1.0, 0.0};
    Vec<double, 3> w = {0.0, 0.0, 1.0};
};
...
CubicAnisotropySetting read_anisotropy_setting_cube(Setting &setting, std::string order_name) {
    ...
    if (setting.isList()) {
        ...
        result.u = jams::normalize(Vec<double, 3>{setting[1][0], setting[1][1], setting[1][2]});
        result.v = jams::normalize(Vec<double, 3>{setting[2][0], setting[2][1], setting[2][2]});
        result.w = jams::normalize(Vec<double, 3>{setting[3][0], setting[3][1], setting[3][2]});
...
jams::Real CubicAnisotropyHamiltonian::calculate_energy(const int i, jams::Real time) {
    ...
    Vec<double, 3> spin = Vec<double, 3>{globals::s(i, 0), globals::s(i, 1), globals::s(i, 2)};
    Vec<double, 3> u = {u_axes_(i, 0), u_axes_(i, 1), u_axes_(i, 2)};
    Vec<double, 3> v = {v_axes_(i, 0), v_axes_(i, 1), v_axes_(i, 2)};
    Vec<double, 3> w = {w_axes_(i, 0), w_axes_(i, 1), w_axes_(i, 2)};
...
jams::Real CubicAnisotropyHamiltonian::calculate_energy_difference(int i, const Vec<double, 3> &spin_initial,
...
Vec<jams::Real, 3> CubicAnisotropyHamiltonian::calculate_field(const int i, jams::Real time) {
