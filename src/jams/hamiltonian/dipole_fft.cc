...
namespace {
    const Mat<jams::Real, 3, 3> Id = {1, 0, 0, 0, 1, 0, 0, 0, 1};
}
...
jams::Real DipoleFFTHamiltonian::calculate_energy(const int i, jams::Real time) {
  const Vec<jams::Real, 3> s_i = jams::array_cast<jams::Real>(Vec<double, 3>{globals::s(i, 0), globals::s(i, 1), globals::s(i, 2)});
    ...
    int i, const Vec<double, 3> &spin_initial, const Vec<double, 3> &spin_final, jams::Real time)
{
    ...
    calculate_fields(time);
    for (auto m = 0; m < 3; ++m) {
        Vec<int, 3> pos = ::globals::lattice->cell_offset(i);
...
Vec<jams::Real, 3> DipoleFFTHamiltonian::calculate_field(const int i, jams::Real time) {
...
DipoleFFTHamiltonian::generate_kspace_dipole_tensor(const int pos_i, const int pos_j, std::vector<Vec<double, 3>> &generated_positions) {
  ...
  const Vec<double, 3> r_cart_i = globals::lattice->fractional_to_cartesian(r_frac_i);
