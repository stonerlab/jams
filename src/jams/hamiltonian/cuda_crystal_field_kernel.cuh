__global__ void cuda_crystal_field_energy_kernel(const unsigned int num_spins, const double * dev_s, const double* dev_cf_coeffs, jams::Real * dev_e) {
...
__global__ void cuda_crystal_field_kernel(const unsigned int num_spins, const double * dev_s, const double* dev_cf_coeffs, jams::Real * dev_h) {
    const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
    const unsigned int base = 3u * idx;
    if (idx >= num_spins) return;

    double sx = dev_s[base + 0];
    double sy = dev_s[base + 1];
    double sz = dev_s[base + 2];
