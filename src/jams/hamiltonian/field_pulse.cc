...
namespace {
    ...
    public:
        GaussianSurfacePulse(
            ...
            const jams::Real& temporal_center, const Vec<jams::Real, 3>& field) :
        ...
        : GaussianSurfacePulse(
            ...
            jams::config_required<Vec<jams::Real, 3>>(settings, "field")) {};

        Vec<jams::Real, 3> local_field(const jams::Real& time, const Vec<jams::Real, 3>& r) override {
        ...
        Vec<jams::Real, 3> max_field(const jams::Real& time) override {
          if (time == cache_time_) {
            return cache_field_;
          }
          cache_time_ = time;
        ...
        bool count_as_surface(const Vec<jams::Real, 3>& r) const {
    ...
    private:
        ...
        Vec<jams::Real, 3> field_;
        Vec<jams::Real, 3> cache_field_ = {0.0, 0.0, 0.0};
...
Vec<jams::Real, 3> FieldPulseHamiltonian::calculate_field(int i, jams::Real time) {
  Vec<jams::Real, 3> r {globals::positions(i, 0), globals::positions(i, 1), globals::positions(i, 2)};
...
jams::Real FieldPulseHamiltonian::calculate_energy(int i, jams::Real time) {
  Vec<jams::Real, 3> spin = jams::array_cast<jams::Real>(Vec<double, 3>{globals::s(i,0), globals::s(i, 1), globals::s(i, 2)});
  Vec<jams::Real, 3> field = calculate_field(i, time);
