...
Vec<jams::Real, 3> CrystalFieldHamiltonian::calculate_field(int i, jams::Real time) {
  const double sx = globals::s(i, 0);
  const double sy = globals::s(i, 1);
  const double sz = globals::s(i, 2);

  Vec<double, 3> h = {0.0, 0.0, 0.0};
...
jams::Real CrystalFieldHamiltonian::calculate_energy_difference(int i, const Vec<double, 3> &spin_initial, const Vec<double, 3> &spin_final,
                                                            jams::Real time) {
  return crystal_field_energy(i, spin_final) - crystal_field_energy(i, spin_initial);
}
jams::Real CrystalFieldHamiltonian::crystal_field_energy(int i, const Vec<double, 3> &s) {
  if (!spin_has_crystal_field_(i)) {
    return 0.0;
  }
