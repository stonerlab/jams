...
DipoleNeighbourListHamiltonian::DipoleNeighbourListHamiltonian(const libconfig::Setting &settings, const unsigned int size)
  ...
  std::vector<Vec<jams::Real, 3>> positions;
  positions.reserve(globals::num_spins);
  for (auto i = 0; i < globals::num_spins; ++i)
  {
    positions.push_back(Vec<jams::Real, 3>{globals::positions(i,0), globals::positions(i,1), globals::positions(i,2)});
  }
  neartree.insert_sites(positions);

  std::size_t max_memory_per_tensor = (sizeof(std::vector<std::pair<Vec<double, 3>,int>>*) + sizeof(Vec<double, 3>) + sizeof(int));
  ...
  std::cout << "  dipole neighbour list memory (estimate) "
            << memory_in_natural_units(max_memory_per_tensor * globals::num_spins * neartree.num_neighbours(
                Vec<jams::Real, 3>{globals::positions(0, 0), globals::positions(0, 1), globals::positions(0, 2)}, r_cutoff_)) << std::endl;
  ...
  for (auto i = 0; i < neighbour_list_.size(); ++i) {
    ...
    auto all_neighbours = neartree.neighbours(Vec<jams::Real, 3>{globals::positions(i, 0), globals::positions(i, 1), globals::positions(i, 2)}, r_cutoff_);
...
jams::Real DipoleNeighbourListHamiltonian::calculate_energy_difference(int i, const Vec<double, 3> &spin_initial, const Vec<double, 3> &spin_final, jams::Real time) {
...
[[gnu::hot]]
Vec<jams::Real, 3> DipoleNeighbourListHamiltonian::calculate_field(const int i, jams::Real time)
{
  ...
  Vec<jams::Real, 3> field = {0.0, 0.0, 0.0};
  for (const auto & neighbour : neighbour_list_[i])
  {
    int j = neighbour.second;
    if (j == i) continue;

    Vec<jams::Real, 3> s_j = jams::array_cast<jams::Real>(Vec<double, 3>{globals::s(j,0), globals::s(j,1), globals::s(j,2)});
    Vec<jams::Real, 3> r_ij =  neighbour.first - r_i;
