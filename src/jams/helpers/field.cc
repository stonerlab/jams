	...
	Array<complex<double>, 4>& output) 	  // FFT'd 3D array where 4th dim is the unitcell position
{
	...
	Array<complex<double>, 1> exp_phase_x(lattice->kspace_size()[0]);
	Array<complex<double>, 1> exp_phase_y(lattice->kspace_size()[1]);
	Array<complex<double>, 1> exp_phase_z(lattice->kspace_size()[2]);
	...
	fftw_plan plan = fftw_plan_dft_3d(
		lattice->kspace_size()[0], 
		lattice->kspace_size()[1], 
		lattice->kspace_size()[2], 
    ...
    auto r_cart = lattice->fractional_to_cartesian(lattice->motif_atom(n).pos);
        ...
        jblib::Array<complex<double>, 4> &out_z)
{
	...
	Array<complex<double>, 3> remapped_x(lattice->kspace_size()[0], lattice->kspace_size()[1], lattice->kspace_size()[2]);
