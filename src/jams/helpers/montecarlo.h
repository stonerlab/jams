...
namespace jams {
    namespace montecarlo {
        ...
        /// moves. The class works as a functor which accepts a spin (jams::Vec<double, 3>) and
        /// returns an altered (moved) spin (jams::Vec<double, 3>) which will be used as a trial
        ...
        public:
            virtual jams::Vec<double, 3> operator()(const jams::Vec<double, 3>& spin) = 0;
        };
        ...
        public:
            inline jams::Vec<double, 3> operator()(const jams::Vec<double, 3>& spin) {
              return -spin;
            }
        };
        ...
        /// Get spin 'i' from the global spin array as a jams::Vec<double, 3>
        inline jams::Vec<double, 3> get_spin(int i) {
            return {globals::s(i,0), globals::s(i,1), globals::s(i,2)};
        }

        /// Set spin 'i' in the global spin array to 'spin'
        inline void set_spin(const int &i, const jams::Vec<double, 3>& spin) {
        ...
        inline bool accept_on_boltzmann_distribution(const double& deltaE, const double& temperature, const double prefactor = 1.0) {
          ...
          double beta = 1.0 / (kBoltzmannIU * temperature);
