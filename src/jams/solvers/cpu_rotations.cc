  ...
  if (rotate_all_spins_) {
    ...
    for (auto m = 0; m < num_theta_; ++m) {
      ...
      for (auto n = 0; n < num_phi_; ++n) {
        ...
        jams::Mat<double, 3, 3> rotation = rotation_matrix_z(-phi)*rotation_matrix_y(-theta);
    ...
    for (auto i = 0; i < globals::lattice->num_basis_sites(); ++i) {
      ...
      jams::Vec<double, 3> spin_initial = {globals::s(i, 0), globals::s(i, 1),
                           globals::s(i, 2)};
      ...
      for (auto m = 0; m < num_theta_; ++m) {
        ...
        for (auto n = 0; n < num_phi_; ++n) {
          ...
          // rotate one spin
          jams::Vec<double, 3> spin = jams::spherical_to_cartesian_vector(1.0, theta, phi);
          for (auto j = 0; j < 3; ++j) {
            globals::s(i, j) = spin[j];
          }
