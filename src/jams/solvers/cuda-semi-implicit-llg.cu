...
#include "jams/solvers/cuda-semi-implicit-llg.h"
...
#include "jams/solvers/cuda-semi-implicit-llg-kernel.h"

#include "jblib/containers/array.h"

void CUDASemiImplicitLLGSolver::initialize(int argc, char **argv, double idt)
{
  using namespace globals;

  CudaSolver::initialize(argc, argv, idt);

  ::output->write("\ninitializing CUDA Semi Implicit LLG solver\n");
...
void CUDASemiImplicitLLGSolver::run()
{
  using namespace globals;
  ...
  cuda_semi_implicit_llg_kernelA<<<grid_size, block_size>>>
  ...
  cuda_semi_implicit_llg_kernelB<<<grid_size, block_size>>>
      (dev_s_.data(), dev_ds_dt_.data(), dev_s_old_.data(),
        dev_h_.data(), thermostat_->noise(),
        dev_gyro_.data(), dev_alpha_.data());
...
CUDASemiImplicitLLGSolver::~CUDASemiImplicitLLGSolver()
{
