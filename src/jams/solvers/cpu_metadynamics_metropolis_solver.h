...
public:
    ...
    double energy_difference(const int spin_index, const Vec<double, 3> &initial_Spin,
                             const Vec<double, 3> &final_Spin) override;
    ...
    // been accepted.
    void accept_move(const int spin_index, const Vec<double, 3> &initial_spin,
                     const Vec<double, 3> &final_spin) override;
};
