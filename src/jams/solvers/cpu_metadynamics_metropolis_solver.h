...
class MetadynamicsMetropolisSolver {
 ...
 private:
  ...
  __unused static void intialise_potential_histograms(std::vector<double> &potential_1D, std::vector<std::vector<double>> &potential_2D,const std::vector<double> sample_points_1d,const std::vector<double> sample_points_2d, const std::vector<double> sample_points_m_perpendicular);
  ...
  static bool floats_are_equal(const double& x, const double& y, const double epsilon = 1e-8);
