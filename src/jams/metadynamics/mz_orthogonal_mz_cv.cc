...
void jams::MzOrthogonalMzCV::output() {
  ...
  if (solver->iteration() % 1000000 == 0) {
	...
	double jams::MzOrthogonalMzCV::potential_difference(int i, const Vec3 &spin_initial, const Vec3 &spin_final) {
	  ...
	  auto initial_potential =
		  interpolated_2d_potential(initial_magnetisation[2] / globals::num_spins, initial_m_perpendicular);
	  ...
	  auto trial_potential =
		  interpolated_2d_potential(trial_magnetisation[2] / globals::num_spins, trial_m_perpendicular);
	...
	double jams::MzOrthogonalMzCV::gaussian_2D(const double &x,
											   const double &x0,
											   const double &y,
											   const double &y0,
