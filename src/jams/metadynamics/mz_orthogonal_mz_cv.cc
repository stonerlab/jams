...
void jams::MzOrthogonalMzCV::output(std::ofstream &of) {

  assert(of.is_open());
  for (auto i=0; i<sample_points_mz_ .size(); ++i){
	for(auto ii=0; ii<sample_points_mz_perpendicular_.size();++ii){
	  of << sample_points_mz_perpendicular_[ii] << "	" << sample_points_mz_[i] << "	" << potential_2d_[i][ii] << "\n";
	}}
}
...
double jams::MzOrthogonalMzCV::potential_difference(int i, const Vec3 &spin_initial, const Vec3 &spin_final) {
  ...
  Vec3 trial_magnetisation = magnetisation_ - spin_initial + spin_final; // TODO: check which is normalised and which is not
...
void jams::MzOrthogonalMzCV::insert_gaussian( const double &relative_amplitude) {
  ...
  gaussian_amplitude_ = amplitude_tempering(magnetisation_[2]/globals::num_spins,mz_perpendicular_); //weird. 
  ...
  for (int i = 0; i < sample_points_mz_.size(); ++i) {
	for (int ii = 0; ii < sample_points_mz_perpendicular_.size(); ++ii) {
	  potential_2d_[i][ii] += gaussian_2D(magnetisation_[2]/globals::num_spins,sample_points_mz_[i],mz_perpendicular_,sample_points_mz_perpendicular_[ii]);
...
double jams::MzOrthogonalMzCV::interpolated_2d_potential( const double& m, const double m_p) {
  assert(m <= 1);
  assert(m_p <1);
  double lower_m = floor(abs((m - sample_points_mz_[0]) / (sample_points_mz_[1] - sample_points_mz_[0]))); //lower_y index for the discrete_potential
  double upper_m = lower_m+1;
  double lower_m_p = floor(abs((m_p - sample_points_mz_perpendicular_[0]) / (sample_points_mz_perpendicular_[1] - sample_points_mz_perpendicular_[0])));//lower_x index for the discrete_potential
  ...
  double R1 = jams::maths::linear_interpolation(m_p, sample_points_mz_perpendicular_[lower_m_p], Q11, sample_points_mz_perpendicular_[upper_m_p], Q21);
  double R2 = jams::maths::linear_interpolation(m_p,sample_points_mz_perpendicular_[lower_m_p],Q12,sample_points_mz_perpendicular_[upper_m_p],Q22);
...
double jams::MzOrthogonalMzCV::amplitude_tempering(const double &m, const double &m_p) {
  return gaussian_amplitude_*exp(-interpolated_2d_potential(magnetisation_[2]/globals::num_spins,mz_perpendicular_)/kBoltzmann*bias_temp_);


}

double jams::MzOrthogonalMzCV::mz_perpendicular(Vec3 &magnetisation) {
  return sqrt(((magnetisation[0]/globals::num_spins)*(magnetisation[0]/globals::num_spins))+((magnetisation[1]/globals::num_spins)*(magnetisation[1]/globals::num_spins)));
}

double jams::MzOrthogonalMzCV::gaussian_2D(const double &x, const double &x0, const double &y,const double &y0) const {
  return gaussian_amplitude_*exp(- (((x-x0)*(x-x0))+((y - y0)*(y - y0)))/(2.0 * gaussian_width_*gaussian_width_));
}
