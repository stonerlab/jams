...
jams::MetadynamicsPotential::MetadynamicsPotential(
  ...
  for (auto i = 0; i < num_cvars; ++i) {
    ...
    if (cvar_names_[i] == ("skyrmion_coordinate_x") ||
      ...
      auto bottom_left = globals::lattice->get_unitcell().matrix() * Vec<double, 3>{0.0, 0.0, 0.0};
      auto bottom_right = globals::lattice->get_unitcell().matrix() *
                          Vec<double, 3>{double(globals::lattice->size(0)), 0.0, 0.0};
      auto top_left = globals::lattice->get_unitcell().matrix() *
                      Vec<double, 3>{0.0, double(globals::lattice->size(1)), 0.0};
      auto top_right = globals::lattice->get_unitcell().matrix() *
                       Vec<double, 3>{double(globals::lattice->size(0)), double(
...
void jams::MetadynamicsPotential::spin_update(int i, const Vec<double, 3> &spin_initial,
                                              const Vec<double, 3> &spin_final) {
...
double jams::MetadynamicsPotential::potential_difference(
    int i, const Vec<double, 3> &spin_initial, const Vec<double, 3> &spin_final) {
