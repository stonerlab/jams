...
class MetadynamicsPotential {
  public:
    ...
    enum class PotentialBCs {
      ...
      RestoringBC
      // For values bigger than a threshold -> NO Gaussians are deposited, returns a V_{restoring}(Q(x)) potential.
    };
    ...
    std::array<double, kNumCVars> cvar_coordinates() const;
  ...
  private:
    ...
    void add_gaussian_to_landscape(const std::array<double, kNumCVars> center,
    ...
    std::array<int, kNumCVars> potential_grid_indices(const std::array<double, kNumCVars> &cvar_coordinates) const;
...
#endif //JAMS_METADYNAMICS_POTENTIAL_H
