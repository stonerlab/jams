...
class CVarSkyrmionCoreCoordinate : public CachingCollectiveVariable {
 public:
  ...
  double spin_move_trial_value(int i, const Vec<double, 3> &spin_initial, const Vec<double, 3> &spin_trial) override;
 ...
 private:
  ...
  Vec<double, 3> center_of_mass_reverse_transform(const double total_mass, const Vec<double, 3>& tube_center_of_mass_x, const Vec<double, 3>& tube_center_of_mass_y);
...
//  double skyrmion_center_of_mass_coordinate_y();
  ...
  static bool is_spin_below_threshold(const Vec<double, 3> &s_initial, const Vec<double, 3> &s_final, const double &threshold);
  ...
  std::vector<Vec<double, 3>> cylinder_remapping_x_;
  std::vector<Vec<double, 3>> cylinder_remapping_y_;
