...
//  cuda_reduced_mz_kernel<<<grid_size, block_size>>>(globals::num_spins, m[0], m[1], m[2], derivatives_.device_data());
  ...
  const double dx = -(1/globals::num_spins) * m[0]*m[2] / pow3(norm(m));
  const double dy = -(1/globals::num_spins) * m[1]*m[2] / pow3(norm(m));
  const double dz = (1/globals::num_spins) * (pow2(norm(m)) - m[2]*m[2]) / pow3(norm(m));
