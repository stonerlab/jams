...
jams::CVarTopologicalChargeFiniteDiff::CVarTopologicalChargeFiniteDiff(
  ...
  // ------------------------------- ∂S/∂x -------------------------------------
  {
    ...
    std::vector<std::pair<jams::Vec<double, 3>, double>> dx_interaction_data;

    if (stencil == FiniteDifferenceStencil::Square8) {
      dx_interaction_data = {
          {jams::Vec<double, 3>{ 1, 1, 0}, 1.0/16.0},
          {jams::Vec<double, 3>{ 1,-1, 0}, 1.0/16.0},
          {jams::Vec<double, 3>{ 1, 0, 0}, 3.0/8.0},
          {jams::Vec<double, 3>{-1, 0, 0},-3.0/8.0},
          {jams::Vec<double, 3>{-1, 1, 0},-1.0/16.0},
          {jams::Vec<double, 3>{-1,-1, 0},-1.0/16.0},
      };
    ...
    if (stencil == FiniteDifferenceStencil::Hexagonal4) {
      dx_interaction_data = {
          {jams::Vec<double, 3>{ 1,-1, 0}, 1.0/2.0},
          {jams::Vec<double, 3>{ 0, 1, 0}, 1.0/2.0},
          {jams::Vec<double, 3>{-1, 1, 0},-1.0/2.0},
          {jams::Vec<double, 3>{ 0,-1, 0},-1.0/2.0},
      };
    }
    if (stencil == FiniteDifferenceStencil::Hexagonal6) {
      dx_interaction_data = {
          {jams::Vec<double, 3>{ 1, 0, 0}, 1.0/3.0},
          {jams::Vec<double, 3>{-1, 0, 0},-1.0/3.0},
          {jams::Vec<double, 3>{ 1,-1, 0}, 1.0/6.0},
          {jams::Vec<double, 3>{-1, 1, 0},-1.0/6.0},
          {jams::Vec<double, 3>{ 0, 1, 0}, 1.0/6.0},
          {jams::Vec<double, 3>{ 0,-1, 0},-1.0/6.0}
      };
    }
    if (stencil == FiniteDifferenceStencil::Hexagonal12) {
      dx_interaction_data = {
          {jams::Vec<double, 3>{ 1, 0, 0}, 1.0/2.0}, // F
          {jams::Vec<double, 3>{-1, 0, 0},-1.0/2.0}, // G
          {jams::Vec<double, 3>{ 1,-1, 0}, 1.0/4.0}, // C
          {jams::Vec<double, 3>{-1, 1, 0},-1.0/4.0}, // D
          {jams::Vec<double, 3>{ 0, 1, 0}, 1.0/4.0}, // B
          {jams::Vec<double, 3>{ 0,-1, 0},-1.0/4.0}, // E
          {jams::Vec<double, 3>{ 1, 1, 0},-1.0/12.0},
          {jams::Vec<double, 3>{ 2,-1, 0},-1.0/12.0},
          {jams::Vec<double, 3>{-2, 1, 0}, 1.0/12.0},
          {jams::Vec<double, 3>{-1,-1, 0}, 1.0/12.0}
      };
    }
    ...
    jams::InteractionList<jams::Mat<double, 3, 3>,2> nbrs = neighbour_list_from_interactions(interaction_template);
  ...
  // ------------------------------- ∂S/∂y -------------------------------------
  {
    // first index is interaction vector, second is the +/- sign of the
    // contribution
    std::vector<std::pair<jams::Vec<double, 3>, double>> dy_interaction_data;

    if (stencil == FiniteDifferenceStencil::Square8) {
      dy_interaction_data = {
          {jams::Vec<double, 3>{ 1, 1, 0}, 1.0/16.0},
          {jams::Vec<double, 3>{-1, 1, 0}, 1.0/16.0},
          {jams::Vec<double, 3>{ 0, 1, 0}, 3.0/8.0},
          {jams::Vec<double, 3>{ 0,-1, 0},-3.0/8.0},
          {jams::Vec<double, 3>{ 1,-1, 0},-1.0/16.0},
          {jams::Vec<double, 3>{-1,-1, 0},-1.0/16.0},
      };
    ...
    if (stencil == FiniteDifferenceStencil::Hexagonal4) {
      dy_interaction_data = {
          {jams::Vec<double, 3>{-1, 1, 0}, 1.0/(2*sqrt(3.0))},
          {jams::Vec<double, 3>{ 0, 1, 0}, 1.0/(2*sqrt(3.0))},
          {jams::Vec<double, 3>{ 1,-1, 0},-1.0/(2*sqrt(3.0))},
          {jams::Vec<double, 3>{ 0,-1, 0},-1.0/(2*sqrt(3.0))},
      };
    }
    if (stencil == FiniteDifferenceStencil::Hexagonal6) {
      dy_interaction_data = {
          {jams::Vec<double, 3>{ 1,-1, 0},-sqrt(3.0)/6.0},
          {jams::Vec<double, 3>{-1, 1, 0}, sqrt(3.0)/6.0},
          {jams::Vec<double, 3>{ 0, 1, 0}, sqrt(3.0)/6.0},
          {jams::Vec<double, 3>{ 0,-1, 0},-sqrt(3.0)/6.0},
      };
    }
    if (stencil == FiniteDifferenceStencil::Hexagonal12) {
      dy_interaction_data = {
          {jams::Vec<double, 3>{ 1,-1, 0},-sqrt(3.0)/4.0},
          {jams::Vec<double, 3>{-1, 1, 0}, sqrt(3.0)/4.0},
          {jams::Vec<double, 3>{ 0, 1, 0}, sqrt(3.0)/4.0},
          {jams::Vec<double, 3>{ 0,-1, 0},-sqrt(3.0)/4.0},
          {jams::Vec<double, 3>{-1, 2, 0}, -1/(6*sqrt(3))},
          {jams::Vec<double, 3>{ 1,-2, 0}, 1/(6*sqrt(3))},
          {jams::Vec<double, 3>{ 1, 1, 0},-1.0/(12*sqrt(3))},
          {jams::Vec<double, 3>{ 2,-1, 0}, 1.0/(12*sqrt(3))},
          {jams::Vec<double, 3>{-2, 1, 0}, -1.0/(12*sqrt(3))},
          {jams::Vec<double, 3>{-1,-1, 0}, 1.0/(12*sqrt(3))}
      };
    }
    ...
    jams::InteractionList<jams::Mat<double, 3, 3>, 2> nbrs = neighbour_list_from_interactions(
