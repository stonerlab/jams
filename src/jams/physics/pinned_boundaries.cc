...
: Physics(settings) {
  for (const auto& [name, comp] : boundary_definitions_) {
    ...
    boundary.magnetisation = jams::config_required<jams::Vec<double, 3>>(settings, name + "_pinned_magnetisation");
...
void PinnedBoundariesPhysics::update(const int &iterations, const double &time, const double &dt) {
    for (const auto& [name, boundary] : boundaries) {
      if (globals::solver->is_cuda_solver()) {
        jams::Vec<double, 3> mag = jams::vector_field_indexed_scale_and_reduce_cuda(globals::s, globals::mus, boundary.indices);
        ...
        jams::Vec<double, 3> mag = jams::sum_spins_moments(globals::s, globals::mus, boundary.indices);
        auto rotation_matrix = rotation_matrix_between_vectors(mag, boundary.magnetisation);
