...
protected:
    ...
    void init_test(const jams::Vec<double, 3>& a, const jams::Vec<double, 3>& b, const jams::Vec<double, 3>& c, const jams::Vec<bool, 3>& pbc, const int& supercell_size, const double& r_cutoff, const double& epsilon) {
      ...
      std::vector<jams::Vec<double, 3>> sites;
      int count = 0;
...
//        std::cout << "near tree neighbours:    " << near_tree_neighbours.size() << "\n";
    ...
    jams::Vec<double, 3> a_;
    jams::Vec<double, 3> b_;
    jams::Vec<double, 3> c_;
    jams::Vec<bool, 3> pbc_;
    int supercell_size_;
    double r_cutoff_;
    double epsilon_;
...
TEST_F(InteractionNeartreeTest, neighbours_no_pbc_simple) {
  ...
  jams::Vec<double, 3> a = {1.0, 0.0, 0.0};
  jams::Vec<double, 3> b = {0.0, 1.0, 0.0};
  jams::Vec<double, 3> c = {0.0, 0.0, 1.0};
  jams::Vec<bool, 3> pbc = {false, false, false};
...
TEST_F(InteractionNeartreeTest, neighbours_no_pbc_complicated) {
  ...
  jams::Vec<double, 3> a = {0.5, 0.0, 0.0};
  jams::Vec<double, 3> b = {-0.25, 0.4330127019, 0.0};
  jams::Vec<double, 3> c = {0.0, 0.2886751346, 0.4082482905};
  jams::Vec<bool, 3> pbc = {false, false, false};
  int superlattice_size = 5;
  double r_cutoff = 2.0;
  const double epsilon = 1e-5;
