...
class LatticeMinimumImageTest : public ::testing::TestWithParam<std::tuple<jams::Vec<double, 3>, jams::Vec<double, 3>, jams::Vec<double, 3>>> {
protected:
    ...
    std::vector<jams::Vec<double, 3>> positions = {
        // origin
        jams::Vec<double, 3>{0.0, 0.0, 0.0},

        // body center
        jams::Vec<double, 3>{0.5, 0.5, 0.5},

        // edges
        jams::Vec<double, 3>{0.25, 0.0, 0.0},
        jams::Vec<double, 3>{0.0, 0.25, 0.0},
        jams::Vec<double, 3>{0.0, 0.0, 0.25},

        jams::Vec<double, 3>{0.5, 0.0, 0.0},
        jams::Vec<double, 3>{0.0, 0.5, 0.0},
        jams::Vec<double, 3>{0.0, 0.0, 0.5},

        jams::Vec<double, 3>{0.75, 0.0, 0.0},
        jams::Vec<double, 3>{0.0, 0.75, 0.0},
        jams::Vec<double, 3>{0.0, 0.0, 0.75},

        // faces
        jams::Vec<double, 3>{0.0, 0.25, 0.25},
        jams::Vec<double, 3>{0.25, 0.0, 0.25},
        jams::Vec<double, 3>{0.25, 0.25, 0.0},
        jams::Vec<double, 3>{0.25, 0.25, 0.0},

        jams::Vec<double, 3>{0.0, 0.5, 0.5},
        jams::Vec<double, 3>{0.5, 0.0, 0.5},
        jams::Vec<double, 3>{0.5, 0.5, 0.0},
        jams::Vec<double, 3>{0.5, 0.5, 0.0},

        jams::Vec<double, 3>{0.0, 0.75, 0.75},
        jams::Vec<double, 3>{0.75, 0.0, 0.75},
        jams::Vec<double, 3>{0.75, 0.75, 0.0},
        jams::Vec<double, 3>{0.75, 0.75, 0.0},

        // quadrants
        jams::Vec<double, 3>{0.25, 0.25, 0.25},
        jams::Vec<double, 3>{0.25, 0.25, 0.75},
        jams::Vec<double, 3>{0.25, 0.75, 0.25},
        jams::Vec<double, 3>{0.75, 0.25, 0.25},
        jams::Vec<double, 3>{0.75, 0.75, 0.25},
        jams::Vec<double, 3>{0.75, 0.25, 0.75},
        jams::Vec<double, 3>{0.25, 0.75, 0.75},
        jams::Vec<double, 3>{0.75, 0.75, 0.75},

        // thirds
        jams::Vec<double, 3>{1.0/3.0, 1.0/3.0, 1.0/3.0},
        jams::Vec<double, 3>{2.0/3.0, 2.0/3.0, 2.0/3.0},
    };
};
...
TEST_P(LatticeMinimumImageTest, minimum_image_smith_method) {
  ...
  jams::Vec<double, 3> a = std::get<0>(GetParam());
  jams::Vec<double, 3> b = std::get<1>(GetParam());
  jams::Vec<double, 3> c = std::get<2>(GetParam());

  jams::Vec<bool, 3> pbc = {true, true, true};

  for (auto i = 0; i < positions.size(); ++i) {
    for (auto j = 0; j < positions.size(); ++j) {
      const jams::Vec<double, 3> r_i = a * positions[i][0] + b * positions[i][1] + c * positions[i][2];
      const jams::Vec<double, 3> r_j = a * positions[j][0] + b * positions[j][1] + c * positions[j][2];

      jams::Vec<double, 3> r_ij_smith, r_ij_brute;
      ...
      const jams::Vec<::testing::Matcher<double>, 3> result = {
...
INSTANTIATE_TEST_SUITE_P(LatticeTypes, LatticeMinimumImageTest,
                         testing::Values(
                             std::tuple<jams::Vec<double, 3>, jams::Vec<double, 3>, jams::Vec<double, 3>>({1,0,0}, {0,1,0}, {0,0,1}),             // cubic
                             std::tuple<jams::Vec<double, 3>, jams::Vec<double, 3>, jams::Vec<double, 3>>({1,0,0}, {0.5,sqrt(3)/2,0}, {0,0,1.2}), // hexagonal
                             std::tuple<jams::Vec<double, 3>, jams::Vec<double, 3>, jams::Vec<double, 3>>({0.5, -1/(2*sqrt(3)), 1.0/3.0 }, {1, 1/sqrt(3), 1.0/3.0}, {-0.5, -1/(2*sqrt(3)), 1.0/3.0}), // rhombohedral
                             std::tuple<jams::Vec<double, 3>, jams::Vec<double, 3>, jams::Vec<double, 3>>({1,0,0}, {0,1,0}, {0,0,0.98}),             // tetragonal
                             std::tuple<jams::Vec<double, 3>, jams::Vec<double, 3>, jams::Vec<double, 3>>({1,0,0}, {0,0.75,0}, {0,0,1.2})             // orthorhombic
                         ));
