...
// in JAMS, so by undefining __ICC, the FFTW header will ignore the quad routines.
#ifdef __ICC
#include <fftw3.h>
#else
#define __ICC
...
namespace jams {
    namespace fftw {
        inline fftw_complex* complex_cast(std::complex<double>* ptr) {
          return reinterpret_cast<fftw_complex*>(ptr);
        }

        inline fftw_complex* complex_cast(Vec3cx* ptr) {
          return reinterpret_cast<fftw_complex*>(ptr);
        }
    }
}
...
#define FFTW_CONST_DOUBLE_CAST(x) reinterpret_cast<fftw_complex*>(x)
...
void fft_supercell_vector_field_to_kspace(const jams::MultiArray<double, 2>& rspace_data, jams::MultiArray<Vec3cx,4>& kspace_data, const Vec3i& kspace_size, const Vec3i& kspace_padded_size, const int & num_sites, int fftw_threads = 1);
