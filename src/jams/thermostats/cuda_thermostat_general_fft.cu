...
void CudaGeneralFFTNoiseGenerator::update() {
  ...
  if (filter_temperature_ != this->temperature()) {
    throw std::runtime_error(
        "you cannot dynamically change the temperature for the CudaGeneralFFTNoiseGenerator");
  }
  ...
  const auto start_index =
      num_channels_even_ * pbc(iteration_ + num_trunc_ + 1, 2 * num_trunc_ + 1);
  CHECK_CURAND_STATUS(curandGenerateNormalDouble(
      jams::instance().curand_generator(),
      white_noise_.device_data() + start_index,
      num_channels_even_,
      0.0,
      1.0));
...
double CudaGeneralFFTNoiseGenerator::stationary_variance() const {
  if (num_trunc_ == 0) {
    return 0.0;
  }
...
void CudaGeneralFFTNoiseGenerator::output_diagnostics(
...
void CudaGeneralFFTNoiseGenerator::output_thermostat_properties(std::ostream& os) const {
...
std::vector<double> CudaGeneralFFTNoiseGenerator::discrete_real_fourier_transform(
    std::vector<double>& x) {
...
void CudaGeneralFFTNoiseGenerator::init_lorentzian(
    const CudaGeneralFFTNoiseGeneratorConfig& config) {
  if (std::isnan(config.lorentzian_gamma) || std::isnan(config.lorentzian_omega0)) {
    throw std::runtime_error("lorentzian spectra require thermostat.lorentzian_gamma and thermostat.lorentzian_omega0");
  }
...
double CudaGeneralFFTNoiseGenerator::classical_ohmic_spectrum(
    const double omega,
    const double temperature) {
  return 2.0 * kBoltzmannIU * temperature;
}

double CudaGeneralFFTNoiseGenerator::quantum_ohmic_spectrum(
    const double omega,
    const double temperature) {
  if (omega == 0.0) {
    return 2.0 * kBoltzmannIU * temperature;
...
double CudaGeneralFFTNoiseGenerator::quantum_no_zero_ohmic_spectrum(
    const double omega,
    const double temperature) {
  if (omega == 0.0) {
    return 2.0 * kBoltzmannIU * temperature;
...
double CudaGeneralFFTNoiseGenerator::classical_lorentzian_spectrum(
    const double omega,
    const double temperature,
    const double omega0,
    const double gamma,
...
double CudaGeneralFFTNoiseGenerator::quantum_lorentzian_spectrum(
    const double omega,
    const double temperature,
    const double omega0,
    const double gamma,
  ...
  const double x = (kHBarIU * std::abs(omega)) / (2.0 * kBoltzmannIU * temperature);
  return kHBarIU * jams::maths::lorentzian(std::abs(omega), omega0, gamma, A) * jams::maths::coth(x);
}

double CudaGeneralFFTNoiseGenerator::quantum_no_zero_lorentzian_spectrum(
    const double omega,
    const double temperature,
    const double omega0,
    const double gamma,
