...
public:
    ...
    jams::Vec<double, 3> a1() const;
    jams::Vec<double, 3> a2() const;
    jams::Vec<double, 3> a3() const;

    const Cell& get_supercell();
    const Cell& get_unitcell();
    const jams::Mat<double, 3, 3>& get_global_rotation_matrix();

    jams::Vec<double, 3> displacement(const jams::Vec<double, 3> &position_i_cart, const jams::Vec<double, 3> &position_j_cart) const;

    jams::Vec<double, 3> displacement(const unsigned& lattice_site_i, const unsigned&lattice_site_j) const;

    bool is_periodic(int dimension) const;
    const jams::Vec<bool, 3> & periodic_boundaries() const;
    ...
    const jams::Vec<double, 3> & lattice_site_position_cart(int lattice_site_index) const;             // cartesian position in the supercell
    const jams::Vec<double, 3> & lattice_site_vector_frac(int lattice_site_index) const;             // fractional position in the supercell

    const std::vector<jams::Vec<double, 3>>& lattice_site_positions_cart() const;
    ...
    const std::vector<jams::Mat<double, 3, 3>>& lattice_site_point_group_symops(int lattice_site_index);
    ...
    const jams::Vec<int, 3>& cell_offset(int lattice_site_index) const {
    ...
    const jams::Vec<double, 3>& cell_center(int cell_index) const {
    ...
    // TODO: remove rmax
    const jams::Vec<double, 3> & rmax() const;

    jams::Vec<double, 3> generate_cartesian_lattice_position_from_fractional(const jams::Vec<double, 3> &basis_site_position_frac,
                                                             const jams::Vec<int, 3> &lattice_translation_vector) const;

    jams::Vec<double, 3> generate_image_position(const jams::Vec<double, 3> &unit_cell_cart_pos, const jams::Vec<int, 3> &image_vector) const;

    // Generates a list of points symmetric to r_cart based on the local point group symmetry of basis_site_index
    std::vector<jams::Vec<double, 3>> generate_symmetric_points(int basis_site_index, const jams::Vec<double, 3> &r_cart, const double &tolerance);

    jams::Vec<double, 3> cartesian_to_fractional(const jams::Vec<double, 3> &r_cart) const;

    jams::Vec<double, 3> fractional_to_cartesian(const jams::Vec<double, 3> &r_frac) const;
    ...
    bool is_a_symmetry_complete_set(int motif_index, const std::vector<jams::Vec<double, 3>> &points, const double &tolerance);
...
private:
    ...
    void global_rotation(const jams::Mat<double, 3, 3> &rotation_matrix);

    void global_reorientation(const jams::Vec<double, 3> &reference, const jams::Vec<double, 3> &vector);

    void calc_symmetry_operations();

    bool symops_enabled_;

    jams::Mat<double, 3, 3> global_orientation_matrix_ = kIdentityMat3;
    ...
    std::vector<jams::Vec<double, 3>> lattice_site_positions_cart_;
    std::vector<jams::Vec<double, 3>> lattice_site_positions_frac_;

    std::vector<int>   lattice_site_to_cell_lookup_;     // index is a spin and the data is the unit cell that spin belongs to
    std::vector<jams::Vec<double, 3>>  cell_centers_;
    ...
    jams::Vec<double, 3> rmax_;

    SpglibDataset *spglib_dataset_ = nullptr;

    std::vector<jams::Mat<double, 3, 3>> sym_rotations_;
    std::vector<jams::Vec<double, 3>> sym_translations_;

    std::vector<std::vector<jams::Mat<double, 3, 3>>> basis_site_point_group_symops_;
};
...
namespace jams {
    ...
    //
    double maximum_interaction_length(const jams::Vec<double, 3>& a1, const jams::Vec<double, 3>& a2, const jams::Vec<double, 3>& a3, const jams::Vec<bool, 3>& periodic_boundaries);
}
