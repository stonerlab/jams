...
jams::Vec<double, 3> Lattice::a1() const {
  return unitcell.a1();
}

jams::Vec<double, 3> Lattice::a2() const {
  return unitcell.a2();
}

jams::Vec<double, 3> Lattice::a3() const {
  return unitcell.a3();
}
...
const jams::Vec<double, 3> &
Lattice::lattice_site_position_cart(int lattice_site_index) const {
  return lattice_site_positions_cart_[lattice_site_index];
}

jams::Vec<double, 3>
Lattice::displacement(const jams::Vec<double, 3> &position_i_cart, const jams::Vec<double, 3> &position_j_cart) const {
...
jams::Vec<double, 3> Lattice::displacement(const unsigned &lattice_site_i, const unsigned &lattice_site_j) const {
  return jams::minimum_image(supercell.a1(),
                             supercell.a2(),
...
jams::Vec<double, 3>
Lattice::cartesian_to_fractional(const jams::Vec<double, 3> &r_cart) const {
  return unitcell.inverse_matrix() * r_cart;
}

jams::Vec<double, 3>
Lattice::fractional_to_cartesian(const jams::Vec<double, 3> &r_frac) const {
...
const jams::Vec<bool, 3> & Lattice::periodic_boundaries() const {
  return lattice_periodic;
}

const jams::Vec<int, 3> &Lattice::kspace_size() const {
  return kspace_size_;
}
...
void Lattice::read_unitcell_from_config(const libconfig::Setting &settings) {
  ...
  auto basis = jams::config_required<jams::Mat<double, 3, 3>>(settings, "basis");
...
void Lattice::init_unit_cell(const libconfig::Setting &lattice_settings, const libconfig::Setting &unitcell_settings) {
  ...
  if (lattice_settings.exists("orientation_axis")) {
    ...
    if (lattice_settings.exists("orientation_lattice_vector")) {
      ...
      auto lattice_vector = jams::config_required<jams::Vec<double, 3>>(lattice_settings, "orientation_cartesian_vector");
...
void Lattice::global_rotation(const jams::Mat<double, 3, 3>& rotation_matrix) {
...
void Lattice::global_reorientation(const jams::Vec<double, 3> &reference, const jams::Vec<double, 3> &vector) {

  jams::Vec<double, 3> orientation_cartesian_vector = jams::normalize(unitcell.matrix() * vector);
  ...
  jams::Vec<double, 3> rotated_orientation_vector = global_orientation_matrix_ * orientation_cartesian_vector;
...
{
  jams::Vec<int, 3> kmesh_size = {lattice_dimensions_[0], lattice_dimensions_[1], lattice_dimensions_[2]};
...
jams::Vec<double, 3> Lattice::generate_cartesian_lattice_position_from_fractional(
    const jams::Vec<double, 3> &basis_site_position_frac,
    const jams::Vec<int, 3> &lattice_translation_vector) const
{
...
jams::Vec<double, 3> Lattice::generate_image_position(
        const jams::Vec<double, 3> &unit_cell_cart_pos,
        const jams::Vec<int, 3> &image_vector) const
{
  jams::Vec<double, 3> frac_pos = cartesian_to_fractional(unit_cell_cart_pos);
  for (int n = 0; n < 3; ++n) {
    if (is_periodic(n)) {
...
void Lattice::calc_symmetry_operations() {
  ...
  jams::Mat<double, 3, 3> rot;
  jams::Vec<double, 3> trans;
  jams::Mat<double, 3, 3> id = {1, 0, 0, 0, 1, 0, 0, 0, 1};
...
bool Lattice::apply_boundary_conditions(jams::Vec<int, 3>& pos) const {
    for (int l = 0; l < 3; ++l) {
...
std::vector<jams::Vec<double, 3>> Lattice::generate_symmetric_points(int basis_site_index, const jams::Vec<double, 3> &r_cart, const double &tolerance = jams::defaults::lattice_tolerance) {
...
const jams::Mat<double, 3, 3> &Lattice::get_global_rotation_matrix() {
...
const jams::Vec<double, 3> &Lattice::lattice_site_vector_frac(int lattice_site_index) const {
  return lattice_site_positions_frac_[lattice_site_index];
}

const std::vector<jams::Vec<double, 3>> &Lattice::lattice_site_positions_cart() const {
  return lattice_site_positions_cart_;
}

const std::vector<jams::Mat<double, 3, 3>> &Lattice::lattice_site_point_group_symops(int lattice_site_index) {
...
double jams::maximum_interaction_length(const jams::Vec<double, 3> &a1, const jams::Vec<double, 3> &a2, const jams::Vec<double, 3> &a3, const jams::Vec<bool, 3>& periodic_boundaries) {
  // 3D periodic
  // -----------
  // This must be the inradius of the parellelepiped
  if (periodic_boundaries == jams::Vec<bool, 3>{true, true, true}) {
  ...
  if (periodic_boundaries == jams::Vec<bool, 3>{true, true, false}) {
    return jams::maths::parallelogram_inradius(a1, a2);
  }
  if (periodic_boundaries == jams::Vec<bool, 3>{true, false, true}) {
    return jams::maths::parallelogram_inradius(a1, a3);
  }
  if (periodic_boundaries == jams::Vec<bool, 3>{false, true, true}) {
  ...
  // dimension.
  if (periodic_boundaries == jams::Vec<bool, 3>{true, false, false}) {
    return 0.5 * jams::norm(a1);
  }
  if (periodic_boundaries == jams::Vec<bool, 3>{false, true, false}) {
    return 0.5 * jams::norm(a2);
  }
  if (periodic_boundaries == jams::Vec<bool, 3>{false, false, true}) {
