...
void neighbour_list_checks(const jams::InteractionList<Mat<double, 3, 3>, 2>& list, const std::vector<InteractionChecks>& checks);
...
namespace { //anon
    ...
    std::optional<int> find_basis_site_index(const Vec<double, 3> &offset, const double tolerance) {
    ...
    Vec<double, 3> lattice_translation_vector(const Vec<double, 3>& r_frac, const double tolerance) {
    ...
    std::optional<int> find_unitcell_partner(int i, Vec<double, 3> r_ij, double tolerance) {
      // returns -1 if no partner is found

      Vec<double, 3> p_i_frac = globals::lattice->basis_site_atom(i).position_frac;
      Vec<double, 3> r_ij_frac = globals::lattice->cartesian_to_fractional(r_ij);
      // fractional interaction vector shifted by motif position
      Vec<double, 3> q_ij = r_ij_frac + p_i_frac;
...
post_process_interactions(std::vector<InteractionData> &interactions, const InteractionFileDescription& desc, CoordinateFormat coord_format, bool use_symops, double energy_cutoff, double radius_cutoff, double distance_tolerance) {
  ...
  apply_transform(interactions, [&](InteractionData J) -> InteractionData {
    Vec<double, 3> p_i_frac = globals::lattice->basis_site_atom(J.basis_site_i).position_frac;
    Vec<double, 3> p_j_frac = globals::lattice->basis_site_atom(J.basis_site_j).position_frac;
    Vec<double, 3> r_ij_frac = globals::lattice->cartesian_to_fractional(J.interaction_vector_cart);
    Vec<double, 3> T = lattice_translation_vector(r_ij_frac + p_i_frac - p_j_frac, distance_tolerance);
...
neighbour_list_from_interactions(std::vector<InteractionData> &interactions) {
  jams::InteractionList<Mat<double, 3, 3>, 2> nbr_list;
    ...
    for (int j = 0; j < globals::lattice->size(1); ++j) {
      for (int k = 0; k < globals::lattice->size(2); ++k) {
        ...
        for (const auto& I : interactions) {
          ...
          Vec<int, 3> d_unit_cell = Vec<int, 3>{i, j, k} + I.lattice_translation_vector;
  ...
  for (const auto& check : checks) {
    switch (check) {
      ...
      if (globals::lattice->is_periodic(0) && globals::lattice->is_periodic(1) && globals::lattice->is_periodic(2)) {
          ...
          auto lambda = [](const Mat<double, 3, 3> &prev,
                           const jams::InteractionList<Mat<double, 3, 3>, 2>::pair_type &next) {
              return prev + next.second;
          };

        std::vector<Mat<double, 3, 3>> motif_position_total_exchange(globals::lattice->num_basis_sites(),
                                                     kZeroMat3);
          for (auto i = 0; i < globals::lattice->num_basis_sites(); ++i) {
          ...
          for (auto i = 0; i < globals::num_spins; ++i) {
            ...
            Mat<double, 3, 3> J0 = std::accumulate(neighbour_list.begin(),
