...
class Physics : public Base {
 public:
  ...
  inline const jams::Vec<double, 3>& applied_field() const { return applied_field_; }
  inline double applied_field(const int i) const { return applied_field_[i]; }

  inline void set_applied_field(const jams::Vec<double, 3> &field) { applied_field_ = field; }

  static Physics* create(const libconfig::Setting &settings);

 protected:
  double  temperature_;
  jams::Vec<double, 3>    applied_field_;
