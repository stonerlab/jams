...
#include "jams/core/sparsematrix4d.h"

#include "jblib/containers/sparsematrix.h"

// // --------------------------------------------------------------------------------------------------------------------------------------------------
// __global__ void cuda_uniaxial_anisotropy_field_kernel(const int num_spins, const CudaFastFloat * dev_d2z_, const CudaFastFloat * dev_d4z_,
//   const CudaFastFloat * dev_d6z_, const CudaFastFloat * dev_sf_, CudaFastFloat * dev_h_) {
//   const int idx = blockIdx.x*blockDim.x+threadIdx.x;
//   if (idx < num_spins) {
//     const CudaFastFloat sz = dev_sf_[3*idx+2];
...
// __global__ void cuda_uniaxial_anisotropy_energy_kernel(const int num_spins, const CudaFastFloat * dev_d2z_, const CudaFastFloat * dev_d4z_,
//   const CudaFastFloat * dev_d6z_, const CudaFastFloat * dev_sf_, CudaFastFloat * dev_e_) {
//   const int idx = blockIdx.x*blockDim.x+threadIdx.x;
//   if (idx < num_spins) {
//     const CudaFastFloat sz = dev_sf_[3*idx+2];
//     dev_e_[idx] = dev_e_[idx] - 0.5*dev_d2z_[idx]*(3.0*sz*sz - 1.0) - 0.125*dev_d4z_[idx]*(35.0*sz*sz*sz*sz - 30.0*sz*sz + 3.0)
//                       - 0.0625*dev_d6z_[idx]*(231.0*sz*sz*sz*sz*sz*sz - 315.0*sz*sz*sz*sz + 105.0*sz*sz - 5.0);
//   }
// }
// // --------------------------------------------------------------------------------------------------------------------------------------------------
// __global__ void cuda_field_to_energy_kernel(const int num_spins, const CudaFastFloat * dev_sf_, const CudaFastFloat * dev_h_, CudaFastFloat * dev_e_) {
//   const int idx = blockIdx.x*blockDim.x+threadIdx.x;
//   if (idx < num_spins) {

//     dev_e_[3*idx+2] = dev_e_[3*idx+2] - 0.5*dev_d2z_[idx]*(3.0*sz*sz - 1.0) - 0.125*dev_d4z_[idx]*(35.0*sz*sz*sz*sz - 30.0*sz*sz + 3.0)
//                       - 0.0625*dev_d6z_[idx]*(231.0*sz*sz*sz*sz*sz*sz - 315.0*sz*sz*sz*sz + 105.0*sz*sz - 5.0);
//   }
// }
// // --------------------------------------------------------------------------------------------------------------------------------------------------
...
__global__ void cuda_realspace_to_kspace_mapping(const CudaFastFloat * real_data, const int * r_to_k_mapping, const int num_spins, const int nkx, const int nky, const int nkz, CudaFastFloat * kspace_data) {
    ...
    const int x = r_to_k_mapping[3*idx+0];
    const int y = r_to_k_mapping[3*idx+1];
    const int z = r_to_k_mapping[3*idx+2];

    kspace_data[((x*nky+y)*nkz+z)*3+0] = real_data[3*idx+0];
    kspace_data[((x*nky+y)*nkz+z)*3+1] = real_data[3*idx+1];
    kspace_data[((x*nky+y)*nkz+z)*3+2] = real_data[3*idx+2];
}
...
__global__ void cuda_kspace_to_realspace_mapping(const CudaFastFloat * kspace_data, const int * r_to_k_mapping, const int num_spins, const int nkx, const int nky, const int nkz, CudaFastFloat * real_data) {
    ...
    real_data[3*idx+0] = kspace_data[((x*nky+y)*nkz+z)*3+0];
    real_data[3*idx+1] = kspace_data[((x*nky+y)*nkz+z)*3+1];
    real_data[3*idx+2] = kspace_data[((x*nky+y)*nkz+z)*3+2];
}
// --------------------------------------------------------------------------------------------------------------------------------------------------
