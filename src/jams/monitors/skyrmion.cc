...
void SkyrmionMonitor::update(Solver& solver) {
    ...
    for (double threshold : thresholds) {
      std::vector<Vec<double, 3> > r_com(globals::lattice->num_materials(), {0.0, 0.0, 0.0});
...
void SkyrmionMonitor::calc_center_of_mass(std::vector<Vec<double, 3> > &r_com, const double &threshold) {
  ...
  std::vector<Vec<double, 3> > tube_x_com(num_types, {0.0, 0.0, 0.0});
  std::vector<Vec<double, 3> > tube_y_com(num_types, {0.0, 0.0, 0.0});
