...
void NeutronScatteringNoLatticeMonitor::output_static_structure_factor() {
  ...
  ofstream ofs = jams::filesystem::open_file(seedname + "_static_structure_factor_path_" + to_string(0) + ".tsv");
...
void NeutronScatteringNoLatticeMonitor::output_neutron_cross_section() {
    ofstream ofs = jams::filesystem::open_file(seedname + "_neutron_scattering_path_" + to_string(0) + ".tsv");
