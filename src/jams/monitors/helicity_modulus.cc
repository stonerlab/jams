...
#if HAS_CUDA
    if (jams::instance().mode() == jams::Mode::GPU) {
        interaction_JR_x_.multiply_gpu(globals::s, entropy_field_x_, jams::instance().cusparse_handle(), cusparse_stream_.get());
        interaction_JR_y_.multiply_gpu(globals::s, entropy_field_y_, jams::instance().cusparse_handle(), cusparse_stream_.get());
        interaction_JR_z_.multiply_gpu(globals::s, entropy_field_z_, jams::instance().cusparse_handle(), cusparse_stream_.get());
      return;
    }
#endif

    interaction_JR_x_.multiply(globals::s, entropy_field_x_);
    interaction_JR_y_.multiply(globals::s, entropy_field_y_);
    interaction_JR_z_.multiply(globals::s, entropy_field_z_);
}
...
Mat3 HelicityModulusMonitor::exchange_total_entropy() {
    ...
    for (auto i = 0; i < globals::num_spins; ++i) {
        ...
        const Vec3 h_i_x = {entropy_field_x_(i,0), entropy_field_x_(i, 1), entropy_field_x_(i, 2)};
