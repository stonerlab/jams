...
void CudaThermalCurrentMonitor::update(Solver& solver) {
  Vec<double, 3> js = execute_cuda_thermal_current_kernel(
...
CudaThermalCurrentMonitor::ThreeSpinList CudaThermalCurrentMonitor::generate_three_spin_from_two_spin_interactions(const jams::InteractionList<Mat<double, 3, 3>, 2>& nbr_list) {
...
//  // Jik * Jjk
  ...
  interaction_matrix_ = jams::InteractionMatrix<Vec<double, 3>, double>(three_spin_list, globals::num_spins);
