    ...
    const Vec<int, 3>& kspace_size) const
{
  ...
  for (auto l = 0; l < kspace_size[0]; ++l)
  {
    for (auto m = 0; m < kspace_size[1]; ++m)
    {
      ...
      {
        const Vec<int, 3> coordinate = {l, m, n};
        const Vec<double, 3> hkl = jams::hadamard_product(coordinate, 1.0 / jams::to_double(kspace_size));
        const Vec<double, 3> xyz = lattice_.get_unitcell().inv_fractional_to_cartesian(hkl);
    ...
    const Vec<int, 3>& kspace_size) const
{
  ...
  std::vector<Vec<double, 3>> hkl_path_nodes(settings.getLength());
  for (auto i = 0; i < settings.getLength(); ++i)
  {
    ...
    hkl_path_nodes[i] = Vec<double, 3>{settings[i][0], settings[i][1], settings[i][2]};
  }
  ...
  {
    const Vec<int, 3> start = jams::to_int(jams::hadamard_product(hkl_path_nodes[i - 1], kspace_size));
    const Vec<int, 3> end = jams::to_int(jams::hadamard_product(hkl_path_nodes[i], kspace_size));
    const Vec<int, 3> displacement = jams::absolute(end - start);
...
void KPointPathBuilder::make_hkl_path(
    const std::vector<Vec<double, 3>>& hkl_nodes,
    const Vec<int, 3>& kspace_size,
    std::vector<jams::HKLIndex>& hkl_path) const
{
  ...
  {
    Vec<int, 3> start = jams::to_int(jams::hadamard_product(hkl_nodes[n], kspace_size));
    Vec<int, 3> end = jams::to_int(jams::hadamard_product(hkl_nodes[n + 1], kspace_size));
    Vec<int, 3> displacement = jams::absolute(end - start);
    ...
    if (displacement[0] >= displacement[1] && displacement[0] >= displacement[2])
    {
      ...
      while (start[0] != end[0])
      {
        const Vec<double, 3> hkl = jams::hadamard_product(start, 1.0 / jams::to_double(kspace_size));
        const Vec<double, 3> xyz = lattice_.get_unitcell().inv_fractional_to_cartesian(hkl);
    ...
    const Vec<double, 3> hkl = jams::hadamard_product(end, 1.0 / jams::to_double(kspace_size));
    const Vec<double, 3> xyz = lattice_.get_unitcell().inv_fractional_to_cartesian(hkl);
