...
void BinaryMonitor::update(Solver& solver) {
  ...
  if (is_file_overwrite_mode) {
    bin_file.open(jams::output::monitor_filename(name(), "bin"), std::ios::binary | std::ios::trunc);
  } else {
    bin_file.open(jams::output::monitor_filename_series(name(), "bin", outcount), std::ios::binary);
  }
