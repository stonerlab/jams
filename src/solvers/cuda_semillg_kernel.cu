...
/*
    ...
    for(i=0; i<3; ++i) {
      h[i] = 0.0;
      for(j=rowptr[3*idx+i]; j<rowptr[3*idx+i+1]; ++j) {
        k=col[j];
        h[i] = h[i] + s_dev[k]*jij_val[j];
      }
    }
*/
    ...
    /*
    h[0] = ( h[0] + ( w_dev[3*idx]*sigma + h_app_x)*mus_dev[idx] )*gyro_dev[idx];
    h[1] = ( h[1] + ( w_dev[3*idx+1]*sigma + h_app_y)*mus_dev[idx] )*gyro_dev[idx];
    h[2] = ( h[2] + ( w_dev[3*idx+2]*sigma + h_app_z)*mus_dev[idx] )*gyro_dev[idx];
    */
