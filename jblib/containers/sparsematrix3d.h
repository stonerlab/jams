...
namespace jblib{
  template < typename type, typename index >
    class Sparsematrix < type, 3, index>
    {
      ...
      { }
        ...
        JB_INLINE Vec3<int> getVec3(const index i) const;
  ...
  template < typename type, typename index >
    JB_INLINE Vec3<int> Sparsematrix<type,3,index>::getVec3(const index i) const{
      assert(checkFinalized());
      return Vec3<int> (indexTable(i,0), indexTable(i,1), indexTable(i,2));
    }
