...
DynamicSFPhysics::DynamicSFPhysics(const libconfig::Setting &settings)
: Physics(settings),
initialized(false),
typeToggle(false),
timePointCounter(0),
...
BZLengths() {
  ...
  settings.lookupValue("componentReal", strReal);
  std::transform(strReal.begin(), strReal.end(), strReal.begin(), toupper);

  componentReal = componentMap[strReal];

  if (settings.exists("componentImag")) {
    settings.lookupValue("componentImag", strImag);
  ...
  if (settings.exists("typewise")) {
    settings.lookupValue("typewise", typeToggle);
    if (typeToggle == true) {
  ...
  // window time
  if (settings.exists("t_window") == true) {
    t_window = settings["t_window"];
  } else {
...
// -----------------------------------------------------------------------------
  ...
  if (typeToggle == true) {
    output.write("  * qSpace allocating %f MB\n",
      (sizeof(fftw_complex)*qDim[0]*qDim[1]*qDim[2]*lattice.numTypes())
      /(1024.0*1024.0));
    ...
    for (int i = 0; i < lattice.numTypes(); ++i) {
      qSpaceFFT.push_back(fftw_plan_dft_3d(qDim[0], qDim[1], qDim[2],
        qSpace+i*qTotal, qSpace+i*qTotal, FFTW_FORWARD, FFTW_MEASURE));
    }
  } else {
    output.write("  * qSpace allocating %f MB\n",
      (sizeof(fftw_complex)*qDim[0]*qDim[1]*qDim[2])/(1024.0*1024.0));
    qSpace = static_cast<fftw_complex*>(
      fftw_malloc(sizeof(fftw_complex)*qDim[0]*qDim[1]*qDim[2]));
...
// -----------------------------------------------------------------------------
  if (typeToggle == true) {
    output.write("  * tSpace allocating %f MB\n", (
      sizeof(fftw_complex)*lattice.numTypes()*nTimePoints*nBZPoints
      /(1024.0*1024.0)));

    tSpace = static_cast<fftw_complex*>(fftw_malloc(
      sizeof(fftw_complex)*lattice.numTypes()*nTimePoints*nBZPoints));
    ...
    tSpace = static_cast<fftw_complex*>(fftw_malloc(
      sizeof(fftw_complex)*nTimePoints*nBZPoints));
...
void DynamicSFPhysics::update(const int &iterations, const double &time, const double &dt) {
  using namespace globals;
    ...
    // Apply cofactors to transform spin components
      if (componentImag == -1) {
        for (int i = 0; i < num_spins; ++i) {
          ...
          coFactors(type, componentReal)*s(i, componentReal);
        ...
        for (int i = 0; i < num_spins; ++i) {
          ...
          qSpace[idx+qTotal*type][0] =
          coFactors(type, componentReal)*s(i, componentReal);
          qSpace[idx+qTotal*type][1] =
          coFactors(type, componentImag)*s(i, componentImag);
        }
      }
...
void DynamicSFPhysics::timeTransform() {
  ...
  const double normTransforms = 1.0/static_cast<double>(nTransforms);
  ...
  if (typeToggle == true) {
    imageSpace = static_cast<double*>(fftw_malloc(
      sizeof(double) * omegaPoints * nBZPoints * lattice.numTypes()));

    for (int i = 0; i < omegaPoints*nBZPoints*lattice.numTypes(); ++i) {
      imageSpace[i] = 0.0;
    }
  } else {
    imageSpace = static_cast<double*>(
      fftw_malloc(sizeof(double)*omegaPoints*nBZPoints));
  ...
  for (int n = 0; n < nTransforms; ++n) {
    ...
    int inembed[] = {static_cast<int>(steps_window)};
    int onembed[] = {static_cast<int>(steps_window)};
    int istride    = nBZPoints;      int ostride    = nBZPoints;
    int idist      = 1;             int odist      = 1;
    fftw_complex* startPtr = (tSpace+n*steps_window*nBZPoints);

    fftw_plan tSpaceFFT = fftw_plan_many_dft(rank, sizeN, howmany, startPtr,
      inembed, istride, idist, startPtr, onembed, ostride, odist,
      ...
      for (int i = 0; i < lattice.numTypes(); ++i) {
        for (unsigned int t = 0; t < steps_window; ++t) {
          for (int q = 0; q < nBZPoints; ++q) {
            ...
            tSpace[tIdx][0] = tSpace[tIdx+i*nBZPoints*nTimePoints][0]
            *FFTWindow(t, steps_window, HAMMING);
            tSpace[tIdx][1] = tSpace[tIdx+i*nBZPoints*nTimePoints][1]
            *FFTWindow(t, steps_window, HAMMING);
          }
        }
    ...
    // normalise transform and apply symmetry -omega omega
        ...
        // assign pixels to image
            ...
            imageSpace[imageIdx+i*omegaPoints*nBZPoints] =
            imageSpace[imageIdx+i*omegaPoints*nBZPoints]
            + (tSpace[tIdx][0]*tSpace[tIdx][0]
              + tSpace[tIdx][1]*tSpace[tIdx][1])*normTransforms;
          }
        }
      }
    } else {
      ...
      for (int t = 0; t < omegaPoints; ++t) {
        for (int q = 0; q < nBZPoints; ++q) {
          ...
          assert(imageIdx >= 0);
          assert(imageIdx < (omegaPoints * nBZPoints));
          imageSpace[imageIdx] = imageSpace[imageIdx]
          + (tSpace[tIdx][0]*tSpace[tIdx][0]
            + tSpace[tIdx][1]*tSpace[tIdx][1])*normTransforms;
        }
      }
  ...
  if (typeToggle == true) {
    for (int i = 0; i < lattice.numTypes(); ++i) {
      ...
      for (int n = 0; n < nBZPoints; ++n) {
        ...
        for (unsigned int omega = 0; omega < ((steps_window/2)+1); ++omega) {
          ...
          DSFFile << lengthTotal << "\t" << BZPoints(q, 0) << "\t"
          << BZPoints(q, 1) << "\t" << BZPoints(q, 2);
          DSFFile << "\t" << omega*freqIntervalSize <<"\t"
          << imageSpace[tIdx+i*omegaPoints*nBZPoints] << "\t"
    ...
    for (int n = 0; n < nBZPoints; ++n) {
      ...
      for (unsigned int omega = 0; omega < ((steps_window/2)+1); ++omega) {
        ...
        DSFFile << lengthTotal << "\t" << BZPoints(q, 0) << "\t"
        << BZPoints(q, 1) << "\t" << BZPoints(q, 2);
        DSFFile << "\t" << omega*freqIntervalSize << "\t" << imageSpace[tIdx]
        << "\t" <<static_cast<double>(BZDegeneracy(n)) << "\n";
...
double DynamicSFPhysics::FFTWindow(const int n, const int nTotal,
  const FFTWindowType type) {
  switch (type) {
    case GAUSSIAN:
      // sigma = 0.4
    return (1.0/(sqrt(2.0*M_PI)*0.4))*exp(-( (
      (static_cast<double>(n)/static_cast<double>(nTotal-1))-0.5)
    * (static_cast<double>(n)/static_cast<double>(nTotal-1))-0.5)/(2.0*0.16));
    break;
    case HAMMING:
    return 0.54 - 0.46*cos((2.0*M_PI*n)/static_cast<double>(nTotal-1));
    break;
    default:
