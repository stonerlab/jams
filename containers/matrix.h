...
namespace jblib {
  ...
  template < typename type >
    JB_INLINE Matrix<type,3,3>::Matrix( const Vec3<type> &a, const Vec3<type> &b, const Vec3<type> &c ) {
  ...
  template < typename type >
    JB_INLINE const Vec3<type>& Matrix<type,3,3>::operator[](int32 index) const {
      return data_[index];
    }

  template < typename type >
    JB_INLINE Vec3<type>& Matrix<type,3,3>::operator[](int32 index){
      return data_[index];
    }
  ...
  template < typename type >
    JB_INLINE Vec3<type> Matrix<type,3,3>::operator*( const Vec3<type> &vec ) const{
      return Vec3<type>(
